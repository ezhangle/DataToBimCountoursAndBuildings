...
namespace DataToBim
{
  class DataToBuilding
  {
    ...
    public DataToBuilding( Document doc, List<Building> buildings, ElementId topographyID, string documentFileAddress = "" )
    {
      ...
      this.document = doc; this.TopographyID = topographyID;
      Transaction createBuildingPadType = new Transaction( this.document, "Create Building Pad Type" );
      createBuildingPadType.Start();
      this.buildingPadType = BuildingPadType.CreateDefault( this.document );
      ...
      for( int i = 0; i < 5; i++ )
      {
        if( elevations[i] != double.NaN && elevations[i] != -1 )
        {
          ...
          if( formCreated )
          {
            Transaction placeFamily = new Transaction( this.document, "Locate a Mass" );
          ...
          {
            this.BuildingFootPrints[i].Visualize( this.document );
          }
        }
      }
      #endregion
      ...
      {
        Transaction placeMass = new Transaction( this.document, "Insert Mass" );
        ...
        try
        {
          FamilyInstance building = this.document.Create.NewFamilyInstance( XYZ.Zero, symbols[i], StructuralType.NonStructural );
          ElementTransformUtils.MoveElement( this.document, building.Id, translations[i] );
      ...
      //{
      //    Transaction CreatePads = new Transaction(this.document, "Create Building Pad");
    ...
    {
      IDictionary<string, string> revitPaths = this.document.Application.GetLibraryPaths();
      string path = "";
    ...
    {
      bool success = true;
      Transaction CreateFamily = new Transaction( this.document, "Create a new Family" );
      CreateFamily.Start();
      Document FamDoc = this.document.Application.NewFamilyDocument( this.FamilyTemplateAddress );
      CreateFamily.Commit();
      ReferenceArray refAr = polygon.Get_ReferenceArray( FamDoc, translation );
      Transaction CreateExtrusion = new Transaction( FamDoc, "Create Extrusion" );
      ...
      //Added by Mohammad
      Transaction AddParamTrans = new Transaction( FamDoc, "Add Parameter" );
      AddParamTrans.Start();
    ...
    {
      Level level = this.document.Create.NewLevel( elevation );
      List<CurveLoop> listOfCurveLoops = new List<CurveLoop>();
      listOfCurveLoops.Add( polygon.Get_CurveLoop() );
      return Autodesk.Revit.DB.Architecture.BuildingPad.Create( this.document, this.buildingPadType.Id, level.Id, listOfCurveLoops );
    }
